    ...
    <Card className="group overflow-hidden border-0 shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-1 bg-white">
      <div className="relative">
        <div className="aspect-[4/3] bg-gradient-to-br from-emerald-100 to-emerald-200 relative overflow-hidden">
          ...
          {/* Difficulty badge */}
          {recipe.difficulty && (
            <Badge className={`absolute top-3 left-3 ${getDifficultyColor(recipe.difficulty)} border-0 font-medium`}>
              {recipe.difficulty}
            </Badge>
          )}
        </div>
        ...
        <CardContent className="p-6">
          ...
          <div className="flex items-center gap-4 text-sm text-neutral-500 mb-4">
            ...
            {recipe.calories_per_serving && (
              <div className="text-emerald-600 font-medium">{recipe.calories_per_serving} cal</div>
            )}
          </div>
          ...
          {recipe.dietary_tags && recipe.dietary_tags.length > 0 && (
            <div className="flex flex-wrap gap-1 mb-4">
              {recipe.dietary_tags.slice(0, 3).map((tag) => (
                <Badge key={tag} variant="secondary" className="text-xs bg-neutral-100 text-neutral-700 border-0">
                  {tag}
                </Badge>
              ))}
              {recipe.dietary_tags.length > 3 && (
                <Badge variant="secondary" className="text-xs bg-neutral-100 text-neutral-700 border-0">
                  +{recipe.dietary_tags.length - 3}
                </Badge>
              )}
            </div>
            ...
            <div className="grid grid-cols-3 gap-2 text-xs text-neutral-600 mb-4">
              {recipe.protein_per_serving && (
                <div className="text-center">
                  <div className="font-medium text-neutral-900">{recipe.protein_per_serving}g</div>
                  <div>Protein</div>
                </div>
              )}
              {recipe.carbs_per_serving && (
                <div className="text-center">
                  <div className="font-medium text-neutral-900">{recipe.carbs_per_serving}g</div>
                  <div>Carbs</div>
                </div>
              )}
              {recipe.fat_per_serving && (
                <div className="text-center">
                  <div className="font-medium text-neutral-900">{recipe.fat_per_serving}g</div>
                  <div>Fat</div>
                </div>
              )}
            </div>
          ...
          <Link href={`/dashboard/recipes/${recipe.id}`}>
            <Button className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-medium">View Recipe</Button>
          </Link>
        </CardContent>
      </div>
    </Card>
