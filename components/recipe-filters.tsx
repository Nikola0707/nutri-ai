...
export function RecipeFilters({ onFiltersChange }: RecipeFiltersProps) {
  ...
  }, [searchQuery, selectedCuisine, selectedDifficulty, selectedDietaryTags])
  ...
  const hasActiveFilters =
    searchQuery || selectedCuisine !== "any" || selectedDifficulty !== "any" || selectedDietaryTags.length > 0
    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-3">
        ...
        <Button
          ...
          className={`border-neutral-200 ${showFilters ? "bg-emerald-50 border-emerald-200 text-emerald-700" : ""}`}
        >
          <Filter size={20} />
          Filters
        </Button>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {selectedCuisine !== "any" && (
            <Badge variant="secondary" className="bg-emerald-100 text-emerald-800">
              {selectedCuisine}
              <X size={14} className="ml-1 cursor-pointer" onClick={() => setSelectedCuisine("any")} />
            </Badge>
          )}
          {selectedDifficulty !== "any" && (
            <Badge variant="secondary" className="bg-emerald-100 text-emerald-800">
              {selectedDifficulty}
              <X size={14} className="ml-1 cursor-pointer" onClick={() => setSelectedDifficulty("any")} />
            </Badge>
          )}
          {selectedDietaryTags.map((tag) => (
            <Badge key={tag} variant="secondary" className="bg-emerald-100 text-emerald-800">
              {tag}
              <X size={14} className="ml-1 cursor-pointer" onClick={() => toggleDietaryTag(tag)} />
            </Badge>
          ))}
      ...
      {showFilters && (
        <div className="bg-white rounded-2xl border border-neutral-200 p-6 space-y-6">
          ...
          {/* Dietary Tags */}
          <div>
            <label className="block text-sm font-medium text-neutral-700 mb-3">Dietary Preferences</label>
